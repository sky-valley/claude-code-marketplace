...
from intent_space_sdk import LocalState, compact_json, now_ms, signup_station
...
class BaseSpaceToolSession:
    ...
    def build_client(self, endpoint: str, local_state: LocalState) -> Any:
    ...
    def connect(self) -> None:
        ...
        if isinstance(enrollment, dict):
            ...
            default_space_id = enrollment.get("commons_space_id") if isinstance(enrollment.get("commons_space_id"), str) else None
            if not isinstance(default_space_id, str):
                default_space_id = enrollment.get("space_id") if isinstance(enrollment.get("space_id"), str) else None
            ...
            self._remember_declared_default_space(default_space_id)
    ...
    def scan(self, space_id: str) -> JsonDict:
        ...
        self.record_step("scan", {"spaceId": space_id, "messageCount": len(scan_result.get("messages", [])), "latestSeq": scan_result.get("latestSeq")})
        return scan_result

    def scan_full(self, space_id: str) -> JsonDict:
        scan_result = self.client.scan_from(space_id, since=0, persist_cursor=False)
        self.record_step("scan_full", {"spaceId": space_id, "messageCount": len(scan_result.get("messages", [])), "latestSeq": scan_result.get("latestSeq")})
        return scan_result
    ...
    def stream_space(
        ...
        *,
        ...
        if not hasattr(self.client, "stream_space"):
            raise RuntimeError("stream_space is not available for this transport")
        events = self.client.stream_space(space_id=space_id, since=since, timeout=timeout, max_events=max_events)
        self.record_step("stream_space", {"spaceId": space_id, "messageCount": len(events)})
        return events

    def sign_challenge(self, challenge: str) -> str:
        return self.local_state.sign_challenge(challenge)

    def signup(self, service_url: str, *, handle: Optional[str] = None) -> JsonDict:
        self.ensure_identity()
        result = signup_station(self.local_state, service_url=service_url, handle=handle or self.agent_name)
    ...
    def record_step(self, name: str, payload: Optional[JsonDict] = None) -> JsonDict:
        self.local_state.ensure_dirs()
        event = {"timestamp": now_ms(), "step": name, "payload": payload or {}}
    ...
    def cursor_state(self) -> JsonDict:
        self.local_state.ensure_dirs()
        return {"endpoint": self.endpoint, "cursors": self.local_state.load_cursors()}
    ...
    def list_artifacts(self) -> List[str]:
        self.local_state.ensure_dirs()
        return sorted(str(path.relative_to(self.workspace)) for path in self.local_state.state_dir.rglob("*") if path.is_file())
    ...
    def snapshot(self, transcript_limit: int = 10, step_limit: int = 10) -> JsonDict:
        return {
            ...
            "currentConnection": {
                ...
                "principalId": self.client.auth.get("principalId") if self.client.auth else None,
    ...
    def promise(self, *, parent_id: str, intent_id: str, content: str, payload: Optional[JsonDict] = None, promise_id: Optional[str] = None) -> JsonDict:
        return create_promise(self.agent_id, parent_id=parent_id, intent_id=intent_id, content=content, payload=payload, promise_id=promise_id)

    def decline(self, *, intent_id: str, parent_id: str, reason: str, payload: Optional[JsonDict] = None) -> JsonDict:
        return create_decline(self.agent_id, intent_id=intent_id, parent_id=parent_id, reason=reason, payload=payload)
    ...
    def complete(self, *, promise_id: str, parent_id: str, summary: str, payload: Optional[JsonDict] = None) -> JsonDict:
        return create_complete(self.agent_id, promise_id=promise_id, parent_id=parent_id, summary=summary, payload=payload)

    def assess(self, *, promise_id: str, parent_id: str, assessment: str, payload: Optional[JsonDict] = None) -> JsonDict:
        return create_assess(self.agent_id, promise_id=promise_id, parent_id=parent_id, assessment=assessment, payload=payload)

    def wait_for_intent(self, space_id: str, *, sender_id: Optional[str] = None, payload_predicate: Optional[Callable[[JsonDict], bool]] = None, wait_seconds: float, scan_attempts: int = 1) -> JsonDict:
        return self.wait_or_scan(space_id, lambda message: self._match_message(message, message_type="INTENT", parent_id=space_id, sender_id=sender_id, payload_predicate=payload_predicate), wait_seconds=wait_seconds, scan_attempts=scan_attempts)

    def wait_for_promise(self, space_id: str, *, sender_id: Optional[str] = None, payload_predicate: Optional[Callable[[JsonDict], bool]] = None, wait_seconds: float, scan_attempts: int = 1) -> JsonDict:
        return self.wait_or_scan(space_id, lambda message: self._match_message(message, message_type="PROMISE", parent_id=space_id, sender_id=sender_id, payload_predicate=payload_predicate), wait_seconds=wait_seconds, scan_attempts=scan_attempts)

    def wait_for_decline(self, space_id: str, *, intent_id: Optional[str] = None, sender_id: Optional[str] = None, wait_seconds: float, scan_attempts: int = 1) -> JsonDict:
        return self.wait_or_scan(space_id, lambda message: self._match_message(message, message_type="DECLINE", parent_id=space_id, sender_id=sender_id) and (intent_id is None or message.get("intentId") == intent_id), wait_seconds=wait_seconds, scan_attempts=scan_attempts)

    def wait_for_complete(self, space_id: str, *, promise_id: Optional[str] = None, sender_id: Optional[str] = None, wait_seconds: float, scan_attempts: int = 1) -> JsonDict:
        return self.wait_or_scan(space_id, lambda message: self._match_message(message, message_type="COMPLETE", parent_id=space_id, sender_id=sender_id) and (promise_id is None or message.get("promiseId") == promise_id), wait_seconds=wait_seconds, scan_attempts=scan_attempts)
    ...
    def wait_or_scan(self, space_id: str, predicate: Callable[[JsonDict], bool], *, wait_seconds: float, scan_attempts: int = 1) -> JsonDict:
        ...
        while time.time() < deadline:
            ...
            try:
                ...
                self.record_step("wait_or_scan.async_match", {"spaceId": space_id, "messageType": message.get("type"), "senderId": message.get("senderId")})
                return message
            except TimeoutError:
                pass
            for _ in range(scan_attempts):
                scan_result = self.scan(space_id)
                self.record_step("wait_or_scan.scan", {"spaceId": space_id, "messageCount": len(scan_result.get("messages", [])), "latestSeq": scan_result.get("latestSeq")})
                match = find_first(scan_result.get("messages", []), predicate)
                if match is not None:
                    self.record_step("wait_or_scan.scan_match", {"spaceId": space_id, "messageType": match.get("type"), "senderId": match.get("senderId")})
                    return match
    ...
    def _match_message(self, message: JsonDict, *, message_type: str, parent_id: Optional[str] = None, sender_id: Optional[str] = None, payload_predicate: Optional[Callable[[JsonDict], bool]] = None) -> bool:
